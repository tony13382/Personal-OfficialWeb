    ...
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-50 top-[15vh] left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden animate-[scale-in_150ms]">
          <Dialog.Title className="sr-only">搜尋文章</Dialog.Title>
            ...
            {results.length > 0 && (
              <ul ref={listRef} className="py-2 list-none !m-0 !p-0">
                {results.map((result, i) => (
                  <li key={result.url + i} className="list-none">
                    <a
                      href={result.url}
                      className={`flex items-start gap-3 px-4 py-3 no-underline text-gray-900 transition-colors group ${activeIndex === i ? "bg-gray-100" : "hover:bg-gray-50"}`}

...
<Image src={apps} alt="Velja's Apps panel routing Figma, Apple Music, Discord and other web services directly to their desktop counterparts" />
...
### Apple Music: Desktop Does the Same Thing Smoother

Apple Music share links (`music.apple.com/*`) open as a web player in the browser — the experience is jarring. Playback takes time to load, skipping tracks doesn't integrate with system background playback, and the web playlist lives completely separately from whatever I was already listening to.

Routing these links straight into Music.app means clicking one just adds a song to the existing playback flow — **playback state stays continuous**. A share link doesn't interrupt my queue, and the whole path feels much smoother.

### Why Some Links Should Skip the Browser

These two examples happen to illustrate two different reasons: **Figma is "desktop can do more," Apple Music is "desktop does the same thing better."** Similar pairings: App Store links routing to App Store.app, Discord invite links routing to Discord.app — same logic throughout. If the desktop app experience is better than the web version, let Velja route it through and save the repeated manual switching.

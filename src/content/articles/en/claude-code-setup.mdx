...
<div class="callout info">If you configured the Status Line earlier, the backup account's `~/.claude-back/settings.json` can point to the same script, for example `"command": "bash ~/.claude/statusline.sh"`. That lets one script serve both accounts while each account keeps its own settings and login state.</div>

<a href="https://code.claude.com/docs/en/env-vars" target="_blank" class="official-link">Claude Code Environment Variables</a>

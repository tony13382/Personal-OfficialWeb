...
<LightboxImage src={githubCreate1} alt="The New button in the top-right of the GitHub profile page" />
...
<LightboxImage src={githubCreate2} alt="The GitHub Create repository form" />
...
<LightboxImage src={cloudflareWorker1} alt="The Cloudflare Workers and Pages dashboard with the Create application button highlighted" />
...
<LightboxImage src={cloudflareWorker2} alt="The Cloudflare deployment method panel including Continue with GitHub" />
...
<LightboxImage src={cloudflareWorker3} alt="Cloudflare picking a repo from a GitHub account" />
...
<LightboxImage src={cloudflareWorker4} alt="Cloudflare's create application screen with build and deploy commands filled in" />
...
<LightboxImage src={cloudflareDomain1} alt="A Cloudflare Workers project's Custom domains tab with the Set up a custom domain button" />
...
<LightboxImage src={cloudflareDomain2} alt="The Cloudflare Add a custom domain form with a domain input and a Continue button" />

## Workspace: Four Spaces, One App Each

Once the site is up, the rest is just looping "write, preview, edit, preview again." If those four actions live in the same window stack, things pile up — every time I need to find something I lose a few seconds picking the right window. After v3 launched, I split my tools across four macOS Spaces. Each Space holds exactly one app in full-screen, and I swipe between them with a three-finger gesture.

<LightboxImage src={workspaceSpaces} alt="Four macOS Spaces each holding one app: Zen Browser, VS Code, Figma, Claude Code" />

Each Space maps to one action:

- **Zen Browser:** (Preview) Dedicated to `localhost:4321` for the local preview. Zen has Workspaces and sidebar tab groups that Chrome and Safari don't — so when I duck out to look something up, the preview tab doesn't get buried
- **VS Code:** (Writing & Coding) MDX articles and Astro / React code both live here. MDX preview, Tailwind hints, and Git are all in the same window
- **Figma:** (Design Image) Article hero images, app covers, and social graphics all get laid out in Figma. One file holds all the assets — I just export whichever frame I need
- **Claude Code:** (Chat for dev & write) A dedicated Space for the Claude Code terminal — questions, style tweaks, running `/new-article` or `/translate-article`, all here

The benefit isn't just visual tidiness — it's that **one Space maps to one action**. Want to see the result? Swipe to Preview. Want to write? Swipe to VS Code. My brain never has to decide "which window do I open." A three-finger swipe is cheaper than scanning a Cmd-Tab list of a dozen windows — checking the result while writing becomes almost reflexive.

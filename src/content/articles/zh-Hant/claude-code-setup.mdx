...
## CC Switch：兩組訂閱帳號之間一鍵切換

如果你跟我一樣訂了兩組 Claude 帳號——主力一個、備用一個，週用量爆掉的時候要無縫接上——原本的切換流程其實很麻煩：登出再登入、或是手改 `~/.claude/settings.json`，改錯一個字元 Claude Code 就直接跑不起來。

<Image src={ccSwitch} alt="CC Switch 介面，並排顯示主力跟備用兩組 Claude 帳號的剩餘用量" />

[CC Switch](https://github.com/farion1231/cc-switch) 是一個桌面 App，把多組帳號設定全部視覺化，每組都看得到 5 小時跟 7 天的剩餘用量、還剩幾小時重置，主力快爆的時候一眼就能判斷該不該切到備用。點一下托盤圖示就完成配置切換，比起手改 `settings.json` 省事很多。

<div class="callout warning">CC Switch 做的事情是「幫你把 settings.json 改好」，所以已經開著的 Claude Code session 要 `/exit` 再 `claude --resume`（或直接關掉 terminal 重開）才會載入新帳號。官方 README 寫的「熱切換」實際上指的是「不用手改設定檔」，不是「session 不用重啟」。</div>

幾個額外的好處：

- **中轉服務也能一起管**：如果你還會走 AWS Bedrock 或社群中轉服務，內建 50+ 預設供應商可以塞進同一組切換邏輯，不用為了不同端點再開另一個工具
- **跨工具統一管理**：Claude Code、Codex、Gemini CLI 都能在同一個介面管，MCP Server 設定共用一份，不用每個 CLI 各自配一遍
- **原子寫入**：切換時不會像手改 JSON 那樣有寫到一半噴掉的風險

支援 Mac / Windows / Linux，MIT 開源免費。Mac 用戶可以直接用 Homebrew 裝：
...
<a href="https://github.com/farion1231/cc-switch/releases" target="_blank" class="official-link">下載 CC Switch</a>
...
<Image src={amphetamine} alt="Amphetamine 防休眠工具" />

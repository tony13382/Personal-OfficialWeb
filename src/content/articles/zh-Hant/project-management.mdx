---
title: 如何用 Notion 管理個人專案跟日常任務：我的 LifeOS 工作流程
description: 從任務分類、Notion LifeOS 配置到行事曆排程，分享我管理個人專案跟日常工作的完整工作流程
date: 2026-03-27
category: 工作流程
...
<Image src={shortcutResultNotion} alt="Notion LifeOS 所有任務視圖，新增的兩筆進到 No Sprint 分組" />

Notion 那邊會自動把兩筆寫進 LifeOS，進「所有任務」的 No Sprint 分組，等後續排入 Sprint。

<Image src={shortcutResultReminders} alt="Reminders 的 Notion Catch 清單，兩筆已完成並附上 notion.so 連結" />

Reminders 這邊則會把原本的兩筆標成已完成並附上對應的 Notion 連結，之後要回頭對照原本的想法進了哪個頁面，直接點連結就到。

<a href="https://www.icloud.com/shortcuts/77198d4f7d824524a4033a809b7d50c1" target="_blank" class="official-link">下載捷徑</a>
...
到 [Notion Integrations 頁面](https://www.notion.so/profile/integrations/internal) 建立一個 Internal Integration，把「內容」權限打開就夠用。如果之後想自己延伸更多功能，可以把讀取、更新跟插入一起開起來。建立完會拿到一組 Internal Integration Token，先複製保留。

**2. 取得任務 DB 的 Data Source ID**

<Image src={notionManageDataSources} alt="從檢視設定開啟 Manage data sources" />

打開你要同步的任務 DB View，點右上角的檢視設定圖示，拉到最下面選 **Manage data sources**。

<Image src={notionCopyDataSourceId} alt="複製 data source ID" />

在對應的 data source 旁邊點「...」，選 **Copy data source ID**。這組 ID 是捷徑寫入的目標位置。

**3. 把整合加到 DB**

<Image src={notionAddConnection} alt="在任務 DB 加上整合" />

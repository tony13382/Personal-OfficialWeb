...
先看我目前的 Claude Code 長什麼樣子：底部三行即時顯示模型、專案分支、Session 跟 Weekly 用量。圓點圖示一目瞭然，Weekly 超量還會自動變紅色警告。這只是其中一個設定，後面會一個一個拆開來聊。
...
# Helper: build a dot-bar of N total circles, K filled
# usage: dot_bar <filled> <total> [red]
dot_bar() {
  local filled=$1 total=$2 use_red=${3:-0} bar="" i
  for (( i=0; i<total; i++ )); do
    if [ "$i" -lt "$filled" ]; then
      [ "$use_red" -eq 1 ] && bar="${bar}${RED}●${RESET}" || bar="${bar}●"
    else bar="${bar}○"; fi
  done
  printf "%s" "$bar"
}
...
# Line 2: Session ●●○○○ 40% (14:30)
line2=""
if [ -n "$five_pct" ]; then
  pct_int=$(printf '%.0f' "$five_pct")
  filled=$(( (pct_int * 5 + 50) / 100 ))
  [ "$filled" -gt 5 ] && filled=5
  bar=$(dot_bar "$filled" 5)
  reset_str=""
  [ -n "$five_rst" ] && reset_str=" ($(format_reset "$five_rst"))"
  line2=$(printf "Session: %s %d%%%s" "$bar" "$pct_int" "$reset_str")
fi

# Line 3: Weekly ●●●○○○○ 43% (7 circles = 7 days, red if over pace)
line3=""
if [ -n "$week_pct" ]; then
  ...
  filled=$(( (time_pct * 7 + 50) / 100 ))
  [ "$filled" -gt 7 ] && filled=7
  use_red=0
  [ "$week_pct_int" -gt "$time_pct" ] && use_red=1
  bar=$(dot_bar "$filled" 7 "$use_red")
  line3=$(printf "Weekly:  %s %d%%" "$bar" "$week_pct_int")
fi
...
- **第二行**：Session 用量，用 5 個圓點顯示比例，括號內是下次重置的時間點
- **第三行**：Weekly 用量，用 7 個圓點對應一週七天。圓點的填充是根據目前在七天窗口中經過的時間比例，而不是用量本身——如果用量百分比超過了時間比例，圓點就會變成紅色，提醒你用量超前了節奏

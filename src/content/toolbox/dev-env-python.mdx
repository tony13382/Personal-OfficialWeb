...
我使用 Python 主要集中在後端開發跟資料處理兩個方向，框架上以 FastAPI 為主。這篇整理了我在開發 Python 專案時的環境配置，包含套件管理、程式碼品質工具、容器化部署等面向。
...
我自己主要是透過 VS Code 裡的 Jupyter 擴充套件來開啟 `.ipynb` 檔案，不需要另外啟動 Jupyter Server，直接在編輯器裡就能執行，跟其他開發工作的切換也比較順暢。如果是在雲端環境，我會使用 <a href="https://deepnote.com" target="_blank">Deepnote</a>，它提供線上的 Notebook 環境，適合在不同裝置上快速開始作業或跟團隊協作。
...
資料處理完之後，通常會需要做視覺化來呈現結果。我主要搭配以下兩個工具：

- **Matplotlib**：Python 最基礎的繪圖工具，可以畫出各種基本的圖表。彈性很高，幾乎任何圖表都能客製化，但語法相對繁瑣。
- **plotnine**：基於 ggplot2 語法的 Python 視覺化套件。如果熟悉 R 語言的 ggplot2，用 plotnine 會非常上手。它的語法結構更直覺，透過圖層疊加的方式來建構圖表，程式碼的可讀性也比 Matplotlib 好。
...
- **資料處理**：Pandas
- **視覺化**：Matplotlib / plotnine（ggplot2）

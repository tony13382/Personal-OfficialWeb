...
<Card>
  <CardContent>
    ...
    <div className="space-y-4">
      ...
      <IconBlock title="鍵盤利用 Web APIs 將文字傳送至 `MindReader Core`" icon="Server" />
      <IconBlock title="`Line Bot` 抓取用戶 ID" icon="MessageCircle" />
      <IconBlock title="根據 ID 與文字進行轉換" icon="RefreshCw" />
      <IconBlock title="傳送轉譯後文字" icon="Send" />
      <IconBlock title="接收使用者以外的回饋文字" icon="Server" />
